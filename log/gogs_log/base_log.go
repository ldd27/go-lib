...
func NewLogger(bufLen int64, mode, config string, level int) {
...
func Trace(skip int, format string, v ...interface{}) {
	for _, logger := range loggers {
		logger.Trace(skip, format, v...)
	}
}

func Debug(skip int, format string, v ...interface{}) {
	for _, logger := range loggers {
		logger.Debug(skip, format, v...)
	}
}

func Info(skip int, format string, v ...interface{}) {
	for _, logger := range loggers {
		logger.Info(skip, format, v...)
	}
}

func Warn(skip int, format string, v ...interface{}) {
	for _, logger := range loggers {
		logger.Warn(skip, format, v...)
	}
}
...
func (l *Logger) writerMsg(skip, level int, msg string) error {
	if l.adapter != "console" && l.level != level {
		return nil
	}
	lm := &logMsg{
		skip:  skip,
...
func (l *Logger) Trace(skip int, format string, v ...interface{}) {
	msg := fmt.Sprintf("[T] "+format, v...)
	l.writerMsg(skip, TRACE, msg)
}

func (l *Logger) Debug(skip int, format string, v ...interface{}) {
	msg := fmt.Sprintf("[D] "+format, v...)
	l.writerMsg(skip, DEBUG, msg)
}

func (l *Logger) Info(skip int, format string, v ...interface{}) {
	msg := fmt.Sprintf("[I] "+format, v...)
	l.writerMsg(skip, INFO, msg)
}

func (l *Logger) Warn(skip int, format string, v ...interface{}) {
	msg := fmt.Sprintf("[W] "+format, v...)
	l.writerMsg(skip, WARN, msg)
}
