...
// understands instances of goaext.ServiceError and returns the status and response body embodied in
...
func ErrorHandler(responseErr error, service *goa.Service, verbose bool) goa.Middleware {
		...
		return func(ctx context.Context, rw http.ResponseWriter, req *http.Request) error {
			...
			if status == http.StatusInternalServerError {
				...
				if !verbose {
					rw.Header().Set("Content-Type", goa.ErrorMediaIdentifier)
					if responseErr == nil {
						responseErr = errors.New(http.StatusText(http.StatusInternalServerError))
					}
					msg := fmt.Sprintf("%s [%s]", responseErr.Error(), reqID)
